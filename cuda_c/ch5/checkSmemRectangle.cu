...
{
    // static shared memeory
    __shared__ int tile[BDIMY][BDIMX];

    unsigned int idx = threadIdx.y * blockDim.x + threadIdx.y;

    unsigned int irow = idx / BDIMY;
    unsigned int icol = idx % BDIMY;
    ...
    // set Row for global memory, read col for shared memory
